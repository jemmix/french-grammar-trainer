...
- **SECTION 27 FULLY VALIDATED (2026-09-03)** — all 20 rules + full `--section 27 --llm` sweep green, promoted & pushed per rule. Judge lessons (glm-5.3-flash/low), on top of the recurring comparison-rule curse (same family as 08-11/08-15):
  - **The gallicism-vs-simple-tense rules (27-19/27-20) are unsatisfiable in both directions**: R=simple tense → « no gallicism involved, tests tense choice »; R=gallicism → « tests the construction, not the distinction ». What passes: R=gallicism (full locution like « est en train de » or the conjugated head like « viens de ») + situational criterion in PROMPT (« pavés encore mouillés », « tu arrives trop tard », « elle est encore brûlante ») + form-broken wrongs (va de / est train de / est en train à / PP after de / sans ne / person faults). 27-20 (synthèse) additionally demands the wrongs be OTHER gallicisms (sens-mismatched viens de / suis en train de as INPUT wrongs fine; as MCQ wrongs they get defended — then fall back to form-broken).
  - INPUT R=conjugated « vais »/« vient » alone flip-flops alignment in the synthèse rule — pass reliably only when R is the full gallicism fragment (« est en train de ») or when phrase forces subject+verb (« R je vais », wrongs je viens de / je suis en train de). MCQ↔INPUT conversion (keeping 20/5 split) was needed twice; 012↔021 swap finally landed.
  - hint-not-trivial fires on the ANSWER appearing in PROMPT/EXPLANATION even for tiny verbs (27-20-022 « vient » in « vient de se produire ») — grep R text in the whole question block before validating.
  - Parenthetical criteria that pair two near-synonyms (« action imminente, décision déjà prise ») read as « competing cues »; keep ONE criterion, ideally copied from sentence evidence.
  - « J' » blanks need vowel-initial answers AND wrongs (elision lint) — for allais-family fills use person faults (allait/allaient) + « allais de », not vais/vas.
  - Dict duplicate keys bit twice more (added « est en train de » alias twice, non-adjacent so adjacent-dedupe replace was a no-op) — grep count before insert, and verify count after.
  - Commits: 27-06 c431659e5, 27-07 d43212ea1, 27-08 885fac181, 27-09 f93ff72b0, 27-10 5c0deb467, 27-11 922174cc9, 27-12 c931b28ec, 27-13 87720c719, 27-14 bfb61a922, 27-15 91e1ba9a4, 27-16 fc96f38da, 27-17 9af1695fd, 27-18 8ff3588e0, 27-19 39d2f7f56, 27-20 5c06845aa.

- **SECTION 26 FULLY VALIDATED (2026-09-03)** — all 20 rules + full `--section 26 --llm` sweep green, promoted & pushed per rule. Judge lessons (glm-5.3-flash/low):
  - Judges read rule titles ULTRA-literally; every question sentence must contain the title's named verbs/constructs. 26-18 (« verbes pour lesquels le gérondif est très fréquent »): sentences with movement/other main verbs (partir en courant, venir en criant) = « general formation, not the rule » — every sentence needed commencer/continuer/finir/passer + the gérondif; which-correct answers had to discriminate the construction (à/de/par + inf vs en + gérondif), not just morphology. 26-19 (participe présent descriptions): register-judgment questions (« lequel relève du registre soutenu ») are cursed — grammatical alternatives always « may be correct »; passing shapes = absolute-clause fills/which-correct where wrongs are form-broken (se couché/PP acordé/imparfait conjugué/dangling sans sujet), plus « Dans quelle phrase le participe « X » est-il correctement employé ? » framing.
  - Bare participle in apposition is ALWAYS defended (La cuisinière émince les oignons, utilisant… / En connaissant…-type state readings, « en »-absolute clauses « Le vent en soufflant… ») — never use grammatical-but-odd forms as wrongs; wrong-is-false only survives form-broken wrongs (en+inf, en+PP, PP acordé, conjugué après en, agreement faults).
  - Verb-family judges hallucinate on: fatiguer (fatigant/fatiguant both defended), provoquer (provoquant « accepted variant » as gérondif!), convaincre (judge invented « convainquant »), fatigant-fatiguant orthography and précédent/précédant as adjective-slot questions = « spelling trap not the rule » — convert to gérondif-frame (En ___ le cortège) or swap verb (épuiser, acquérir→prendre).
  - Semantic-value wrongs (En obtenant for antériorité, En cessant subject-absurd, cesse juxtaposition, en fatiguant subject mismatch) get defended as grammatical — replace with form-broken (Ayant obtenir/Ayant cesser/ayant cessée invariance/« ayant cessé de » cassé).
  - « tout en » opposition MCQs: every alternative (En reconnaissant causal, Ayant reconnu, Reconnaissant) defended → pin wrongs as form-broken (Tout en reconnu/Tout en reconnaître/Tout reconnaissant).
  - INPUT self-containedness: meta-jargon parentheticals (« (même sujet, actions simultanées) », « (sujets différents, simultanéité) », « (verbe pronominal, antériorité…) ») fail; over-pinning also fails (« la construction attendue commence par « tout en » » was 4/6 borderline but passed with « on attend la construction X + participe présent du verbe Y »; « avec la forme composée » = vague; « s'étant + participe passé » pin passed in 26-14 but fails 26-20 alignment as « dictates exact construction » — MCQ instead). Passing INPUT mold for this section: en-in-phrase gérondif fills (En ___ l'escalier, R montant, wrongs monter/montait/monté/montants) or connector-fill with construction-naming prompt.
  - « pendant que » content in gérondif-synthesis rules = « temporal subordination, not this rule » (alignment) even though the title says constructions alternatives — keep pendant-que questions inside 26-16 only.
  - Having+PP absolute INPUTs stay cursed (26-15/26-20) — MCQ only.
  - Keeping TYPE counts green: converting an INPUT→MCQ silently breaks the 25-questions/5-INPUT proportion assert and aborts the whole patch script (nothing written) — assert types BEFORE writing; the batch-abort-leaves-nothing lesson bit 4 more times, always grep-verify after "ok".
  - Dict duplicate-key near-miss: adding « en ouvrant » when it already existed (26-09) — grep before insert; vitest doesn't typecheck so TS1117 only surfaces at husky pre-commit.
  - Commits: 26-09 0d7083a7e, 26-10 5d5cfa1f8, 26-11 230f0f48c, 26-12 81a3a47c5, 26-13 5afd92cbc, 26-14 10712ada0, 26-15 2dfbe2a56, 26-16 eb78b55a5, 26-17 670b56920, 26-18 a0794b811, 26-19 871c377b4, 26-20 4e89bb5e4.
...
## Section 11 lessons (learned while validating 11-01..11-20, 2026-09-03)

- **Identification rules (COD/COI)**: fill-the-blank-with-noun INPUTs fail (test vocabulary); blank-free "type the COD/COI" INPUTs fail hint-not-trivial (answer appears in the phrase). Only surviving INPUT shape: Q/A pronoun-replacement (« Tu vois Marie souvent ? Oui, je ___ vois chaque semaine. »). Spell out « complément d'objet direct (COD) » in prompts — bare jargon fails no-ambiguous-prompts. MCQ twin choices (« à X » + bare « X ») are ambiguous → replace the bare one with « de X ». Passive stems get rejected ("subject, not COD") → use active sources.
- **Antecedent anchors mandatory**: any blank whose pronoun is otherwise under-determined (which gender/number/person?) fails no-ambiguous-prompts and mcq-wrong-is-false. Winning molds: dislocation (« Tes parents ? Il ___ appelle »), or full source sentence in the prompt (« Quelle phrase est correcte pour dire « Il donne la clé à moi » avec les pronoms ? »). Glosses like "(COD : les, COI : lui)" are called cryptic and leak the answer — prefer full source sentences.
- **Gender/number wrongs need pinned referents**: « te le » vs « te la » style wrongs are "grammatical under a different referent" unless the source sentence pins the noun. The source-sentence template solves this; bare-gloss prompts do not.
- **Elision rule (R-side)**: if the stem ends in a vowel-initial context, the correct R must be the ELIDED form (« Il me l'a laissée »); an unelided R (« Il me la ai laissée ») is judged wrong. Conversely elision-lint: `je ___` blanks forbid vowel-initial answers (incl. y/en) → use nous/vous/ils subjects or dislocate; `Que ___` blanks forbid vowel-initial answers → use « Qu'est-ce que tu ___ » with consonant answers, or restructure.
- **Defended "wrong" answers keep recurring**: topicalization (« Lui il… », « Les nous invitons »), imperative reversed order claims (« montre-nous-le »), hériter/jouer/en alternate constructions (« il en a trouvé » for plural nouns is valid partitive!), croire en vs croire à, penser à animate nouns (famille → lui). If a judge can construct ANY valid reading, it gets defended — replace those wrongs with conjugation/infinitive/accord faults or person/number faults.
- **Placement rules (COD/COI before verb)**: proven MCQ mold = which-correct with ALL position faults (post-verb, pre-subject, post-aux, post-infinitive, infinitive-for-participle). A criterion prompt (« Quelle phrase place correctement le pronom COD devant le verbe ? ») only passes when every wrong is a position fault — one conjugation wrong flips it to alignment failure.
- **Double pronouns (11-11..11-13, 11-17)**: source-sentence transformation prompts (« Quelle phrase est correcte pour dire « X » avec les pronoms ? ») with wrongs = wrong order + invalid double-COI combo + post-verb/infinitive/accord fault. For imperative double pronouns (11-14), judges demand FULL hyphenated forms as choices (« Montre-la-nous ! ») — bare combos (« la-nous ») are called "non-existent forms".
- **Pronoun-adverb rules (11-15/11-16/11-19)**: questions must embody the rule's canonical verbs (penser à → y vs penser de → en; s'y habituer reflexive placement). Non-canonical verbs (croire, jouer, hériter, attendre, se fier, rêver, se méfier, prendre soin) get rejected as off-rule EVEN with correct answers. Never state the mapping in the prompt (« penser à → y ») — that's an answer giveaway; the contrast belongs in the WRONG answers + explanations. « douter de » was also rejected as off-rule by some judges — stick to penser/songer/réfléchir + the en/y sibling as the contrast wrong.
- **Synthèse (11-20)**: single-pronoun questions fail alignment ("tests only en/y, not synthesis"); need two-group transformations (COD+COI « Vous les lui avez offerts », COD+y « Vous les y avez rangés », lui+en « Nous lui en avons parlé ») with order/accord/aux faults as wrongs. Mixed-shape choices (pronouns + full sentences) fail mcq-structural/no-ambiguous — keep choice shapes uniform; full-sentence choices only in which-correct molds.
- **INPUT prompts**: « remplace les groupes en gras » fails (no bold in DSL) — name the groups explicitly. « garde X » side-conditions confuse — drop them. INPUT answers that are full sentences need answer-hints dict keys; existing hint labels reused: "pronoms doubles", "pronom", "pronom COD", "pronom COI".
- **Section sweep**: `--section 11` after the last rule confirms every rule file passes from cold cache (fast, no LLM calls) — do this before closing a section.
