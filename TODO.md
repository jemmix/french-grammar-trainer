...
- **LLM validation progress** — sections 01-02 fully validated (6200 checks each, 310/310 per rule) with `glm-5.3-flash` variant `low` (now the validate.ts default, which abends on `glm-5-turbo`). Section 03: all rules pass except 14 questions across the imparfait-vs-PC tense-choice rules (03-12-023, 03-13-010/021/024, 03-14-023/025, 03-15-022/023/025, 03-16-009, 03-17-018, 03-19-008/014/019). Sections 04 and 10-28 still pending LLM validation.

- **Tense-choice catch-22 under glm-5.3-flash (like 08-11/08-15)** — the imparfait-vs-PC INPUT rules (03-12..03-16) flip between two contradictory `no-ambiguous` objections across runs: (a) "Conjuguez dans le temps qui convient" → "doesn't specify which tense"; (b) "à l'imparfait ou au passé composé selon le contexte (action de fond ou événement)" → "technical jargon doesn't map to tenses". Most questions in these rules pass; the residual ~10% flip between (a) and (b) on re-rolls, and the doom mechanism (2 fails → skip) makes bad luck sticky. Options tried: explicit tense naming (fails `question-rule-alignment` for tense-choice rules — "gives away the answer"), criterion cues in various wordings, choice-set naming. Next lever if revisiting: restructure the hardest questions as MCQ (alignment accepts choice-based tense selection) or accept per the 08-15 precedent.
