  ...
  return (
    <>
      {parts.map((part, i) => {
        ...
        if (part.startsWith("`") && part.endsWith("`")) {
          return <code key={i} className="px-1 py-0.5 rounded bg-primary/[.06] text-[13px] font-mono text-ink/75">{part.slice(1, -1)}</code>;
        }
        if (part.startsWith("~~") && part.endsWith("~~")) {
          return <s key={i} className="text-accent/60">{part.slice(2, -2)}</s>;
        }
...
export function ExplanationPanel({
  ...
  const content = explanation ? (
    <>
      <div className="text-[14.5px] text-ink/85 leading-[1.7] mb-5">
        <RenderMiniMarkdown text={explanation.body} />
      </div>
      <div className="text-xs font-medium text-muted/60 uppercase tracking-widest mb-2.5">
        {t.quiz.examples}
      </div>
      <ul className="space-y-2">
        {explanation.examples.map((ex, i) => (
          <li key={i} className="flex gap-2 text-[13.5px] text-ink/75 leading-relaxed">
            <span className="text-primary/30 mt-[3px] shrink-0 select-none" aria-hidden>&#x2022;</span>
  ...
  const closeButton = (
    <button
      onClick={onClose}
      className="shrink-0 p-1 -mr-1 rounded-md text-muted/50 hover:text-ink transition-colors"
    ...
    <>
      ...
      {showDesktop && (
        ...
        >
          <div className="flex items-start justify-between px-5 pt-5 pb-2">
            <h3 className="text-[15px] font-semibold text-ink leading-snug pr-3">{title}</h3>
            {closeButton}
          </div>
      ...
      {showMobile && (
        <>
          {/* Backdrop */}
          <div
            className={`lg:hidden fixed inset-0 z-40 bg-ink/25 transition-opacity duration-300 ${
          ...
          <div
            ...
            className={`
              ...
              bg-surface shadow-[0_-4px_32px_rgba(0,0,0,0.08)]
          ...
          >
            <div className="flex justify-center pt-3 pb-1 shrink-0">
              <div className="w-9 h-[3px] rounded-full bg-chalk" />
            </div>
            <div className="flex items-start justify-between px-5 pt-1 pb-2 shrink-0">
              <h3 className="text-base font-semibold text-ink leading-snug pr-4">{title}</h3>
              {closeButton}
            </div>
