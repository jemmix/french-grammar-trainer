...
describe("TypeScript question proportions", () => {
  ...
  for (const lang of ["fr", "en", "de"]) {
    ...
    describe(`src/data/${lang}`, () => {
      for (const file of files) {
        it(`${file}: every rule has questions divisible by 5 with 20% input`, async () => {
          ...
          if (!section || !section.questions) {
            expect.fail(`Could not load section from ${file}`);
          }

          const ruleCounts = extractFromSection(section);
          const issues = validateRuleProportions(ruleCounts, `src/data/${lang}/${file}`);
