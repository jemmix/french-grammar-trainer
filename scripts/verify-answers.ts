/**
 * Verifies MCQ question/answer pairs by asking a Claude model whether
 ...
 *
 * One claude invocation per question/answer combo.
 *
 * Usage:
 *   npx tsx scripts/verify-answers.ts --model <haiku|sonnet|opus> <file.txt> [<file2.txt> ...]
 *
 * Options:
 *   --model <name>       Required. Claude model to use (haiku, sonnet, opus)
 *   --concurrency <n>    Max parallel claude invocations (default: 10)
 *   --dry-run            Print commands instead of executing them
 *   --output-dir <dir>   Directory for reports (default: gen/verify)
 *
 * Outputs (per file):
 *   gen/verify/<rule-id>-results.txt    — full results per answer
 *   gen/verify/<rule-id>-mismatches.txt — only answers where Claude disagreed with expected
 */
...
if (!model) {
  console.error("Error: --model is required (haiku, sonnet, opus)");
  process.exit(1);
}
if (files.length === 0) {
...
function invokeClaude(prompt: string): Promise<string> {
  return new Promise((resolve, reject) => {
    const child = execFile(
      "claude",
      ["-p", prompt, "--model", model, "--system-prompt", SYSTEM_PROMPT],
      { timeout: 30_000, maxBuffer: 10 * 1024 },
      (err, stdout, stderr) => {
        if (err) {
...
async function processFile(filePath: string): Promise<void> {
  ...
  if (dryRun) {
    for (const job of jobs) {
      ...
      console.log(
        `[${tag}] claude -p ${JSON.stringify(prompt)} --model ${model} --system-prompt ${JSON.stringify(SYSTEM_PROMPT)}`,
      );
    }
    return;
  }
